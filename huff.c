...
int main(int argc, char*argv[]){
	...
	if(2 == argc){
		if(strcmp(argv[1], "--help")){
			printf("Uncnown key %s.\n", argv[1]); return 1;
		}else{
			printf("Enter huff <in file> -c <out file> for comression.\n");
			printf("Enter huff <in file> -x <out file> for uncomression.\n"); 
			printf("Examples: huff input.txt -c archive, huff archive -x output.txt.\n"); 
			return 0;
		}	
	}
	...
	if(4 == argc) {
		...
		if(strcmp(argv[2], "-c") && strcmp(argv[2], "-x")){
			printf("Uncnown command %s. Enter huff --help for help.\n", argv[2]); return 1;
		}
	}
	if(4 < argc) {printf("You enter too many arguments. For help use --help command.\n"); return 1;}
#endif
#ifdef DEBUG
    compress("test.txt", "zipped_test.txt");    
    uncompress("zipped_test.txt", "test2.txt");
#endif
    return EXIT_SUCCESS ;
}
